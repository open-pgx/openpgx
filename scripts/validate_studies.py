...
def find_project_root():
    ...
    for p in candidates:
        schema = p / "docs" / "openpgx.schema.json"
        if not schema.exists():
            schema = p / "mcp-server" / "docs" / "openpgx.schema.json"
        if schema.exists():
...
def load_schema(root):
    """Load the study_contribution $def from the unified schema."""
    schema_paths = [
        root / "docs" / "openpgx.schema.json",
        root / "mcp-server" / "docs" / "openpgx.schema.json",
    ]
    for p in schema_paths:
        if p.exists():
            with open(p) as f:
                unified = json.load(f)
            study_def = unified.get("$defs", {}).get("study_contribution")
            if study_def:
                return study_def
            return unified
    print("ERROR: Cannot find openpgx.schema.json")
    sys.exit(1)
